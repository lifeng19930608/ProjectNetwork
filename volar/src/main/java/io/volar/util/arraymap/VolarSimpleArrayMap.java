...
package io.volar.util.arraymap;
...
/**
 * Base implementation of {@link VolarArrayMap} that doesn't include any standard Java
 ...
 */
public class VolarSimpleArrayMap<K, V> {
    ...
    private void allocArrays(final int size) {
        if (size == (BASE_SIZE * 2)) {
            synchronized (VolarArrayMap.class) {
     ...
     */
    public VolarSimpleArrayMap(int capacity) {
        if (capacity == 0) {
            mHashes = EMPTY_INTS;
            mArray = EMPTY_OBJECTS;
        } else {
     ...
     */
    public VolarSimpleArrayMap(VolarSimpleArrayMap<K, V> map) {
        this();
        if (map != null) {
            putAll(map);
        }
     ...
     */
    public void putAll(VolarSimpleArrayMap<? extends K, ? extends V> array) {
    ...
    public boolean equals(Object object) {
        ...
        if (object instanceof VolarSimpleArrayMap) {
            VolarSimpleArrayMap<?, ?> map = (VolarSimpleArrayMap<?, ?>) object;
            if (size() != map.size()) {
                return false;
            }
